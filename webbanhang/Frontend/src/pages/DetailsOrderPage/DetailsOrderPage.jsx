...
import { WrapperAllPrice, WrapperContentInfo, WrapperHeaderUser, WrapperInfoUser
    , WrapperItem, WrapperItemLabel, WrapperLabel
    , WrapperNameProduct, WrapperProduct, WrapperStyleContent } from './style'
      ...
      <div style={{ width: '1270px', margin: '0 auto', height: '1270px'}}>
        ...
        <WrapperHeaderUser>
          <WrapperInfoUser>
            ...
            <WrapperContentInfo>
              <div className='name-info' style={{paddingBottom: '4px'}}>{data?.shippingAddress?.fullName}</div>
          ...
          {data?.orderItems?.map((order) => {
            return (
              <WrapperProduct>
                ...
                <WrapperItem style={{marginTop: '26px', height: '70px'}}>{convertPrice(order?.price)}</WrapperItem>
                <WrapperItem style={{marginTop: '26px', height: '70px'}}>{order?.amount}</WrapperItem>
                <WrapperItem style={{marginTop: '26px', height: '70px'}}>{order?.discount ? convertPrice(priceMemo * order?.discount / 100) : '0 VND'}</WrapperItem>
